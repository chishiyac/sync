...
const jsonTreeViewRecipe = tv({
  base: cn(
    ...
    '[&_[data-part=branch-control][data-state=open]_[data-kind=preview]]:hidden',
...
const jsonTreeViewTreeRecipe = tv({
  base: cn(
    'flex w-full min-w-0 flex-col **:gap-1',
    'text-muted-foreground text-xs leading-7',
    '[&_svg]:size-4',
    '**:data-[type=string]:text-primary',
    '**:data-[type=number]:text-primary',
    ' **:data-[type=boolean]:text-primary',
    ...
    '**:data-[kind=colon]:mx-0 **:data-[kind=colon]:text-muted-foreground'
  )
})

export { jsonTreeViewRecipe, jsonTreeViewTreeRecipe }
